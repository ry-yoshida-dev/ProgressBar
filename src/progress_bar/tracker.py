...
class ProgressTracker:
    ...
    The tracker resolves a backend once on creation, builds the matching
    reporter when the task starts, and can be reused for several tasks.
    ...
    def __init__(
        ...
        *,
        ...
        self._factory = ReporterFactory(self._environment)
    ...
    def from_settings(
        ...
        *,
        ...
        """Build a tracker from an existing settings object.
        ...
        -------
        ProgressTracker
            Tracker configured with ``settings``.
        """
        return cls(
    ...
    def completed(self) -> int:
        ...
        return 0 if self._reporter is None else self._reporter.completed
    ...
    def start(self) -> None:
        ...
        reporter = self._factory.create(self._backend, self._settings)
        reporter.start()
    ...
    def advance(self, step: int = 1) -> None:
        ...
        self._reporter.advance(step)

    def finish(self) -> None:
        """Finalize the running task, ignoring an idle tracker."""
        if self._reporter is None:
            return
    ...
    def track[ItemT](self, iterable: Iterable[ItemT]) -> Iterator[ItemT]:
        ...
        is_owned_task = self._reporter is None
        if is_owned_task:
            self.start()
        try:
